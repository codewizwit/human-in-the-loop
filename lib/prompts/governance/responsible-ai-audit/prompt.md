<prompt>
  <metadata>
    <id>responsible-ai-audit</id>
    <name>Responsible AI Audit</name>
    <version>2.0.0</version>
    <description>Comprehensive audit framework for evaluating AI outputs against responsible AI principles including accuracy, fairness, transparency, accountability, and safety</description>
    ...
    <alignsWith>
      <document>RESPONSIBLE-AI-PLAYBOOK.md</document>
    </alignsWith>
  </metadata>
  ...
  <examples>
    <example>
      <input>
        <ai_output>AI Code Review Bot that automatically comments on PRs with suggestions:
...
- Runs on every PR with no opt-out mechanism</ai_output>
        <use_case>Automated code review assistance in pull requests</use_case>
        <risk_level>high</risk_level>
        <target_audience>Engineering team of 20 developers (5 junior, 10 mid-level, 5 senior)</target_audience>
        <deployment_context>GitHub PRs for production services</deployment_context>
      </input>
...
- **Success Criteria:**
  ...
  - No increase in bug rates</output>
    </example>
    <example>
    <input>
    <ai_output>AI Test Generator that creates unit tests based on function signatures:
...
- Developers must review and modify before merging</ai_output>
  <use_case>Accelerate test coverage for new TypeScript functions</use_case>
  <risk_level>medium</risk_level>
  <target_audience>Development team writing new features</target_audience>
  <deployment_context>Optional development tool, not required</deployment_context>
  </input>
  <output>### Executive Summary
...
- **Success Criteria:**
  ...
  - ≥80% of generated tests are modified (shows thoughtful review)</output>
    </example>
    </examples>
